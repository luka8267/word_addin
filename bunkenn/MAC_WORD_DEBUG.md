...
- `manifest.commands.xml`
  - `VersionOverrides` はあり
  - command surface は最小限
  - `taskpane.minimal.html` を開くので、`VersionOverrides` 由来の問題を切り分けやすい
- `manifest.icons.xml`
  - `VersionOverrides` と icon resources はあり
  - ただし開く先は `taskpane.minimal.html`
  - icon/resource が原因かどうかを切り分けやすい
...
6. これが成功したら `manifest.commands.xml` に差し替える
7. ribbon button から `taskpane.minimal.html` が開くか確認する
8. それも成功したら `manifest.icons.xml` に差し替える
9. icon 付き ribbon button から `taskpane.minimal.html` が開くか確認する
10. それも成功したら `manifest.full.xml` に差し替える
11. ribbon button から現行の `taskpane.html` が開くか確認する
...
### 4. command manifest では `VersionOverrides/Requirements` が重要

Microsoft Learn の add-in commands ドキュメントでは、`VersionOverrides` 内で command 機能に必要な requirement set を宣言する運用が案内されている。
Mac Word で command 付き manifest が一覧に出ない場合は、`AddinCommands 1.1` の宣言不足をまず疑う。
...
3. `manifest.commands.xml` でも壊れるなら `VersionOverrides` 定義そのものを疑う
4. `manifest.commands.xml` は通るが `manifest.icons.xml` で壊れるなら icon / image resources を疑う
5. `manifest.icons.xml` は通るが `manifest.full.xml` で壊れるなら full taskpane / resources を疑う
6. `taskpane.html` は開くが操作できないなら API 応答を見る
7. API が失敗しているなら WebView より先にバックエンド認証を直す
